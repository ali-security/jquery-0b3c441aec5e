...
test( "non-lowercase boolean attribute lookups should not recurse infinitely", function() {
	...
	// jQuery.attr lowercases the name before it reaches the boolean attrHandle,
	// so exercise the vulnerable getter the way Sizzle does: with the raw name.
	jQuery.each({
		checked: "Checked",
		required: "requiRed",
		autofocus: "AUTOFOCUS"
		...
		try {
			strictEqual( jQuery.find.attr( elem, original ), lowercased,
				"jQuery.find.attr with the '" + this + "' name returns the lowercased name" );
		} catch ( e ) {
			ok( false, "jQuery.find.attr with the '" + this + "' name threw" );
		}
	});

	try {
		strictEqual(
			jQuery( "#qunit-fixture input" ).filter("[Checked]").length,
			jQuery( "#qunit-fixture input" ).filter("[checked]").length,
			"A non-lowercase boolean attribute selector matches the lowercase one" );
	} catch ( e ) {
		ok( false, "Filtering by a non-lowercase boolean attribute selector threw" );
	}
});
